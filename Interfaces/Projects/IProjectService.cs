...
namespace api.Interfaces.Projects
{
  public interface IProjectService
  {
    ...
    Task<PaginateProjectDto?> PaginateAsync(int limit = 20, int page = 1, string? typeId = null, string? search = null, string? responsibleId = null);
    Task CreateAsync(NewProjectDto project);
    Task<ProjectDto?> ReadAsync(string id);
    Task UpdateAsync(string id, UpdateProjectDto project);
